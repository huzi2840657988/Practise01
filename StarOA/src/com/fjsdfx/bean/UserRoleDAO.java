...
/**
 	* A data access object (DAO) providing persistence and search support for UserRole entities.
 			* Transaction control of the save(), update() and delete() operations 
		can directly support Spring container-managed transactions or they can be augmented	to handle user-managed Spring transactions. 
		Each of these methods provides additional information for how to configure it for the desired type of transaction control. 	
	 * @see com.fjsdfx.bean.UserRole
...
public class UserRoleDAO extends BaseHibernateDAO  {
    ...
    public UserRole findById( com.fjsdfx.bean.UserRoleId id) {
        log.debug("getting UserRole instance with id: " + id);
        try {
            UserRole instance = (UserRole) getSession()
                    .get("com.fjsdfx.bean.UserRole", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(UserRole instance) {
        ...
        try {
            List results = getSession()
                    .createCriteria("com.fjsdfx.bean.UserRole")
